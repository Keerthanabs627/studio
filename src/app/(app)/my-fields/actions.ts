...
import {
  getCropManagementAdvice as getCropManagementAdviceFlow,
  type CropManagementInput,
} from '@/ai/flows/crop-management-flow';
import {z} from 'zod';
...
export async function getCropManagementAdvice(
  input: CropManagementInput
): Promise<{data?: CropManagementAdvice; error?: string}> {
  ...
  try {
    ...
    return {
      error:
        'An unexpected error occurred while fetching advice. Please try again later.',
    };
  }
}
